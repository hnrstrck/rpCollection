...
public final class RPTastenfeld {
    ...
    private ArrayList<Integer> intInputPins;
    private ArrayList<Integer> intOutputPins;
    ...
    */
    ...
    /**
     * Erstellt ein neues Objeckt der Klasse RPTastenfeld ohne die Pins zu setzten. Die Pins muessen manuell gesetzt werden mit der Methode @{link #setPins(ArrayList<Integer>, ArrayList<Integer>) setPins}
     */
    public RPTastenfeld() {
        this.gpio = GpioFactory.getInstance();
        this.boolInitialisierungErfolgt = false;
    ...
    */
	...
	this.inputPins = new ArrayList<GpioPinDigitalInput>();
		...
		for(int i = 0; i < pPins.size(); i++) {
		    ...
		    System.out.println("Input-Pin " + pPins.get(i) + " wahrgenommen");
	...
	try {
	    for(int i = 0; i < pPins.size(); i++) {
		System.out.println("Output-Pin " + pPins.get(i) + " wahrgenommen");
